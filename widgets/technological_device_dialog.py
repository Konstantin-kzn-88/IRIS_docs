# widgets/technological_device_dialog.py
...
class TechnologicalDeviceDialog(QDialog):
    """Диалог добавления/редактирования технологического устройства"""

    def __init__(self, db: DatabaseConnection, parent=None, device: TechnologicalDevice = None):
        ...
        if device:
            self.setWindowTitle("Редактирование технологического устройства")
            self.load_device_data()
        else:
            self.setWindowTitle("Добавление технологического устройства")
    ...
    def setup_ui(self):
        ...
        self.name_edit.setPlaceholderText("Введите наименование устройства")
        ...
        self.sub_id_edit.setPlaceholderText("Например: TD-001")
        ...
        form_layout.addRow("Тип устройства *:", self.device_type_combo)
    ...
    def validate_and_accept(self):
        ...
        if not self.name_edit.text().strip():
            QMessageBox.warning(self, "Предупреждение",
                                "Наименование устройства не может быть пустым")
    ...
    def load_device_data(self):
        """Загрузка данных устройства в форму"""
        # Устанавливаем проект
        index = self.project_combo.findData(self.device.project_id)
        if index >= 0:
            self.project_combo.setCurrentIndex(index)

        # Устанавливаем вещество
        index = self.substance_combo.findData(self.device.substance_id)
        if index >= 0:
            self.substance_combo.setCurrentIndex(index)

        self.name_edit.setText(self.device.name)

        if self.device.component_enterprise:
            self.component_edit.setText(self.device.component_enterprise)

        if self.device.sub_id:
            self.sub_id_edit.setText(self.device.sub_id)

        # Тип устройства
        index = self.device_type_combo.findText(self.device.device_type)
        if index >= 0:
            self.device_type_combo.setCurrentIndex(index)

        if self.device.volume is not None:
            self.volume_spin.setValue(self.device.volume)

        self.pressure_spin.setValue(self.device.pressure)
        self.temperature_spin.setValue(self.device.temperature)

        if self.device.degree_filling is not None:
            self.filling_spin.setValue(self.device.degree_filling)

        if self.device.spill_square is not None:
            self.spill_spin.setValue(self.device.spill_square)

        if self.device.expected_casualties is not None:
            self.casualties_spin.setValue(self.device.expected_casualties)

    def get_device_data(self) -> TechnologicalDevice:
        """Получение данных устройства из формы"""
