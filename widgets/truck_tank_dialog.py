...
class TruckTankDialog(QDialog):
    ...
    def __init__(self, db: DatabaseConnection, parent=None, tank: TruckTank = None):
        ...
        if tank:
            self.setWindowTitle("Редактирование автоцистерны")
            self.load_tank_data()
        else:
            self.setWindowTitle("Добавление автоцистерны")
...
# Основные поля
        ...
        self.sub_id_edit.setPlaceholderText("Например: AT-001")
        ...
        self.spill_spin.setRange(0, 1000)
...
# Ожидаемое количество пострадавших
    ...
    def validate_and_accept(self):
        ...
        if not self.name_edit.text().strip():
            QMessageBox.warning(self, "Предупреждение",
                                "Наименование автоцистерны не может быть пустым")
            self.name_edit.setFocus()
            return
    ...
    def load_tank_data(self):
        """Загрузка данных автоцистерны в форму"""
        ...
        # Тип давления
        index = self.pressure_type_combo.findText(self.tank.pressure_type)
        if index >= 0:
            self.pressure_type_combo.setCurrentIndex(index)
    ...
    def get_tank_data(self) -> TruckTank:
        """Получение данных автоцистерны из формы"""
        return TruckTank(
