...
class CompressorDialog(QDialog):
    """Диалог добавления/редактирования компрессора"""

    def __init__(self, db: DatabaseConnection, parent=None, compressor: Compressor = None):
        ...
        if compressor:
            self.setWindowTitle("Редактирование компрессора")
            self.load_compressor_data()
        else:
            self.setWindowTitle("Добавление компрессора")
    ...
    def setup_ui(self):
        ...
        self.component_edit.setPlaceholderText("Например: Цех №1, Площадка А и т.д.")
        form_layout.addRow("Компонент предприятия:", self.component_edit)

        # Идентификатор подсистемы
        self.sub_id_edit = QLineEdit()
        self.sub_id_edit.setPlaceholderText("Например: COMP-001")
        ...
        form_layout.addRow("Тип компрессора *:", self.comp_type_combo)
    ...
    def validate_and_accept(self):
        ...
        if not self.name_edit.text().strip():
            QMessageBox.warning(self, "Предупреждение",
                              "Наименование компрессора не может быть пустым")
            self.name_edit.setFocus()
            return
    ...
    def load_compressor_data(self):
        """Загрузка данных компрессора в форму"""
        # Устанавливаем проект
        index = self.project_combo.findData(self.compressor.project_id)
        if index >= 0:
            self.project_combo.setCurrentIndex(index)

        # Устанавливаем вещество
        index = self.substance_combo.findData(self.compressor.substance_id)
        if index >= 0:
            self.substance_combo.setCurrentIndex(index)

        self.name_edit.setText(self.compressor.name)

        if self.compressor.component_enterprise:
            self.component_edit.setText(self.compressor.component_enterprise)

        if self.compressor.sub_id:
            self.sub_id_edit.setText(self.compressor.sub_id)

        # Тип компрессора
        index = self.comp_type_combo.findText(self.compressor.comp_type)
        if index >= 0:
            self.comp_type_combo.setCurrentIndex(index)

        if self.compressor.volume is not None:
            self.volume_spin.setValue(self.compressor.volume)

        self.pressure_spin.setValue(self.compressor.pressure)
        self.temperature_spin.setValue(self.compressor.temperature)

        if self.compressor.flow is not None:
            self.flow_spin.setValue(self.compressor.flow)

        if self.compressor.time_out is not None:
            self.time_out_spin.setValue(self.compressor.time_out)

        if self.compressor.expected_casualties is not None:
            self.casualties_spin.setValue(self.compressor.expected_casualties)

    def get_compressor_data(self) -> Compressor:
        """Получение данных компрессора из формы"""
        return Compressor(
            id=self.compressor.id if self.compressor else None,
            project_id=self.project_combo.currentData(),
