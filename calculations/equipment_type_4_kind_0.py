...
def calc_for_scenario(
    ...
    # пролив (везде одинаково по исходнику)
    if sc_line in (1, 2, 3, 4, 5, 6):
        result["ov_in_accident_t"] = result["amount_t"] + flow * equipment["shutdown_time_s"] * KG_TO_T

    # пролив или факел (по исходнику)
    if sc_line in (1, 2, 4):
        result["ov_in_hazard_factor_t"] = result["ov_in_accident_t"]

    # -------------------------------------------------------------------------
    # Пролив и испарение
    #   В оригинале "полный пролив" только для scenario_line == 5,
    #   иначе пролив берётся как частичный (spill_area_m2 * SPILL_TO_PART).
    # -------------------------------------------------------------------------
    ...
    if sc_line in (5,):  # испарение для вспышки
    ...
    # -------------------------------------------------------------------------
    # Избыточное давление (в этом типе не используется)
    # -------------------------------------------------------------------------
    result["p_70"] = None
    result["p_28"] = None
    result["p_14"] = None
