...
def calc_for_scenario(
    ...
    result["amount_t"] = (
        ...
        + volume * props.density_gas * (1 - fill_fraction) * KG_TO_T
    )

    if DEBUG:
        print(f'result["amount_t"] = {result["amount_t"]}')

    # Эти переменные используются ниже для факела (4/6)
    liquid_flow = None
    gas_flow = None

    if sc_line in (1, 2, 3, 9):  # авария полная
        result["ov_in_accident_t"] = result["amount_t"]

    if sc_line in (4, 5):  # авария частичная (ниже уровня жидкости)
    ...
    if sc_line in (6, 7, 8):  # авария частичная (выше уровня жидкости)
    ...
    if sc_line in (2,):  # испарение для взрыва
    ...
    if sc_line in (4, 6):  # факельное горение газа/жидкости
        result["ov_in_hazard_factor_t"] = result["ov_in_accident_t"]

    if sc_line in (7,):  # вспышка
        result["ov_in_hazard_factor_t"] = result["ov_in_accident_t"] * MASS_IN_CLOUDE

    if sc_line in (3, 5, 8):  # ликвидация
        result["ov_in_hazard_factor_t"] = 0

    if sc_line in (9,):  # огненный шар
    ...
    elif sc_line in (4, 6, 7):  # факел, вспышка
        result["fatalities_count"] = 0
        result["injured_count"] = 1
    elif sc_line in (9,):  # огненный шар после нагрева
        result["fatalities_count"] = 0
        result["injured_count"] = 1
