...
from calc_method.math_ import calc_toxi, calc_damage
...
class Calculation():
    ...
    def get_zone_and_risk_param(self, project_code: str, scenario_number: str, equipment_name: str, equipment_type: Any,
                                ...
                                S_spill: int, gas_temperature: float, gas_weight: float, gas_flow: float,
                                molecular_weight: float, threshold_dose: float, lethal_dose: float,
        ...
        :param gas_weight:                     - масса газа, кг (первичное облако)
        :param gas_flow:                       - расход газа, кг/с (вторичное облако)
        ...
        # расчитываем зоны (токси)
        threshold_dist, lethal_dist = calc_toxi.Source(ambient_temperature=self.constants.AMBIENT_TEMPERATURE,
                                                       cloud=self.constants.CLOUD,
                                                       wind_speed=self.constants.WIND_VELOCITY,
                                                       is_night=self.constants.IS_NIGHT,
                                                       is_urban_area=self.constants.IS_URBAN_AREA,
                                                       ejection_height=self.constants.EJECTION_HEIGHT,
                                                       gas_temperature=gas_temperature, gas_weight=gas_weight,
                                                       gas_flow=gas_flow, closing_time=self.constants.TIME_OUT_DIVICE,
                                                       molecular_weight=molecular_weight).get_threshold_distances(threshold_dose=threshold_dose, lethal_dose=lethal_dose)
        ...
        probability = failure_set.equipment_failure_rates[equipment_type.value]['categories'][model_type][
                          type_accident] * \
                      tree[type_accident][0][tree[type_accident][1].index('toxi')]
