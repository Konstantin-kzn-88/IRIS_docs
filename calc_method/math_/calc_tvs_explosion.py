...
from ..math_ import calc_probit
from ..math_ import _found_nearest_value
...
class Explosion:
    ...
    def explosion_array(self, class_substance: int, view_space: int, mass: float,
        ...
        while delta_p > 2.9:
            ...
            delta_p = res[0]
            impulse = res[1]
            probit = round(calc_probit.Probit().probit_explosion(delta_p, impulse), 3)
            probability = round(calc_probit.Probit().probability(probit), 3)
            # append
    ...
    def explosion_class_zone(self, class_substance: int, view_space: int, mass: float,
        ...
        classified_zone_array = [70, 28, 14, 5, 2]  # CZA
        radius_CZA = []
        ...
        for CZA in classified_zone_array:
            if len(delta_p_array) == 0:
                ...
                ind = delta_p_array.index(_found_nearest_value.get_nearest_value(delta_p_array, CZA))
