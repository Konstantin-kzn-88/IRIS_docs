...
class Strait_fire:
    ...
    def termal_radiation_array(self, S_spill: float, m_sg: float, mol_mass: float,
        ...
        for i in radius_arr:
            ...
            if i < D_eff:
                ...
                probit = calc_probit.Probit().probit_strait_fire(dist, q_term_arr[radius_arr.index(i)])
                probability = calc_probit.Probit().probability(probit)
    ...
    def termal_class_zone(self, S_spill: float, m_sg: float, mol_mass: float,
        ...
        for CZA in classified_zone_array:
            ind = q_term_array.index(_found_nearest_value.get_nearest_value(q_term_array, CZA))
